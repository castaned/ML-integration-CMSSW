...
import sys
import ROOT
from mini_nanotools import PostProcessor, Module, Collection
import math
import itertools

MAPPING_FILE = "data_processing/mapping.json"  # ruta relativa a PROJECT_DIR (raiz del repo); asume que execute_data_processing.py se corrio desde data_processing/
...
def get_dataset_id(dataset_folder):
    ...
    print(f"Mapping: {dataset_mapping}")
    reverse_mapping = {value: int(key) for key, value in dataset_mapping.items()}
    dataset_id = reverse_mapping[dataset_folder]

    return dataset_id


# --- Configure the certified JSON *here* ---
# If you ship "Cert_Run3_Golden.json" alongside the Slurm job, it lands in the job's workdir
...
def is_data_file(path):
    ...
    t = f.Get("Events")
    if not t:
        raise RuntimeError(f"'Events' tree missing in: {path}")
...
class LeptonFilter(Module):
    ...
    def analyze(self, event):
        ...
        # ---------------ELECTRONES
        good_electrons = sorted(
            [
                ele
                for ele in electrons
                if ele.pt > 10 and abs(ele.eta) < 2.5 and ele.cutBased >= 2
            ],
            key=lambda x: x.pt,
            reverse=True,
        )
        ...
        self.out.fillBranch(
            "nLeptons", len(good_leptons)
        )  # numero de leptones por evento
                ...
                if foundW:
                    ...
                    if (
                        ...
                        and (
                            (
                                l1.highPtId == 2
                                and (l2.highPtId == 1 or l2.highPtId == 2)
                            )
                            or (
                                l2.highPtId == 2
                                and (l1.highPtId == 1 or l1.highPtId == 2)
                            )
                        )
                    ):
